...
class DriftAdapter:
    ...
    def __init__(
        self, learner: OnlineLearner, threshold: float = 0.5, window: int = 50
    ) -> None:
        self.learner = learner
        self.threshold = threshold
        self.errors: collections.deque[int] = collections.deque(maxlen=window)

    async def report(self, truth: int, prediction: int) -> None:
        self.errors.append(int(truth != prediction))
        if (
            len(self.errors) == self.errors.maxlen
            and sum(self.errors) / len(self.errors) > self.threshold
        ):
